...
pair_df = pd.read_csv('pairs_data/candidates/candidate_pairs.csv', index_col=False)
...
def visualize_relationship(sec_a, sec_b, days_1, days_2):
    ...
    pricing_data = pd.read_csv('fin_data/' + pair_industry + '.csv')
...
#    fig = plt.figure()
    ...
    plt.savefig('pairs_data/' + today_date + '/' + pair_industry.replace(' ', '') + 
